...
implicit class BigIntOperate(first: BigNum) extends BigInternal {
  def +(second: BigNum): BigNum = {
    var (pfirst, psecond) = pad(extend(first), extend(second))
    var carry = 0
    for (i <- pfirst.value.length - 1 to 0 by -1) {
      val value: Long = (pfirst.value(i) + psecond.value(i) + carry
        .asInstanceOf[CUnsignedInt]).toLong
    ...
    return reduce(pfirst, reduction)
  }
}
